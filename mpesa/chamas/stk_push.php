...
$stmt_serial = $conn->prepare("SELECT serial_no FROM chamas_pays ORDER BY serial_no DESC LIMIT 1");
$stmt_serial->execute();
...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO chamas_pays (serial_no, chama_name, amount, fee, total, phone_number, status, transaction_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
if (isset($response->ResponseCode) && $response->ResponseCode == 0) {
    try {
        ...
        $stmt2 = $conn->prepare("UPDATE chamas_pays SET transaction_date = ?, CheckoutRequestID = ?, merchant_request_id = ? WHERE id = ?");
