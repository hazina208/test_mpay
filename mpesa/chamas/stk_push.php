...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO chama_pays (serial_no, chama_name, amount, fee, total, phone_number, status, transaction_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
if (isset($response->ResponseCode) && $response->ResponseCode == 0) {
    try {
        ...
        $stmt2 = $conn->prepare("UPDATE chama_pays SET transaction_date = ?, CheckoutRequestID = ?, merchant_request_id = ? WHERE id = ?");
