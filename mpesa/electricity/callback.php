...
if (isset($callback['Body']['stkCallback'])) {
    ...
    try {
        if ($resultCode == 0) { // Success
            ...
            $stmt = $conn->prepare("UPDATE power_bills SET status = ?, transaction_id = ? WHERE CheckoutRequestID = ?");
            ...
            $stmt = $conn->prepare("UPDATE power_bills SET status = ? WHERE CheckoutRequestID = ?");
