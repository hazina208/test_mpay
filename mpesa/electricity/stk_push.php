...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO power_bills (agency, amount, fee, total, phone_number, status, transaction_id) VALUES (?, ?, ?, ?, ?, ?, ?)");
...
$payload = [
    ...
    'CallBackURL' => MPESA_CALLBACK_URL12,
    'AccountReference' => 'POWER BILL REMITTANCES_' . $payment_id,
...
if (isset($response->ResponseCode) && $response->ResponseCode == 0) {
    try {
        ...
        $stmt2 = $conn->prepare("UPDATE power_bills SET transaction_date = ?, CheckoutRequestID = ?, merchant_request_id = ? WHERE id = ?");
