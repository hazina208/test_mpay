...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO school_fee_payments (school, amount, fee, total, student_no, phone_number, status, transaction_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([$school, $amount, $fee, $total, $student_no, $phone_number, $status, $transaction_id]);
...
$payload = [
    ...
    'CallBackURL' => MPESA_CALLBACK_URL9,
    'AccountReference' => 'SCHOOL FEE PAYMENT_' . $payment_id,
    'TransactionDesc' => 'Payment for ' . $school . ' - Student No: ' . $student_no
];
...
if (isset($response->ResponseCode) && $response->ResponseCode == 0) {
    try {
        ...
        $stmt2 = $conn->prepare("UPDATE school_fee_payments SET transaction_date = ?, CheckoutRequestID = ?, merchant_request_id = ? WHERE id = ?");
