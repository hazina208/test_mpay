...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO hospital_payments (hospital, amount, fee, total, phone_number, status, transaction_id) VALUES ( ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([$hospital, $amount, $fee, $total,  $phone_number, $status, $transaction_id]);
...
$payload = [
    ...
    'CallBackURL' => MPESA_CALLBACK_URL8,
    'AccountReference' => 'PAY HOSPITAL' . $payment_id,
    'TransactionDesc' => 'Payment for ' . $hospital
];
...
if (isset($response->ResponseCode) && $response->ResponseCode == 0) {
    try {
        ...
        $stmt2 = $conn->prepare("UPDATE hospital_payments SET transaction_date = ?, CheckoutRequestID = ?, merchant_request_id = ? WHERE id = ?");
