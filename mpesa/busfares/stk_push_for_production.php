...
function calculateFee($amount) {
    ...
    } elseif ($amount >= 1501 && $amount <= 2000.99) {
        return $amount * 0.05;  // 0.5%
    } elseif ($amount >= 2001 && $amount <= 4000.99) {
        return $amount * 0.05;  // 0.5%
    } else {
...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO bus_fares (sacco, amount, fee, total, fleet_no, phone_number, status, transaction_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
$payload = [
    ...
    'Amount' => $amount,  // Charge only the original amount (fee deducted on receiver side)
    'PartyA' => $phone_number,
