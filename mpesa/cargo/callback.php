...
if (isset($callback['Body']['stkCallback'])) {
    ...
    if ($resultCode == 0) { // success
        ...
        $stmt = $conn->prepare("UPDATE cargo_payments SET status=?, transaction_id=? WHERE CheckoutRequestID=?");
        $stmt->bind_param("sss", $status_complete, $mpesaReceiptNumber, $checkoutRequestID);
        $stmt->execute();
    } else {
        // Failed transaction
        require_once '../../connection.php';
        $stmt = $conn->prepare("UPDATE cargo_payments SET status=? WHERE CheckoutRequestID=?"); 
