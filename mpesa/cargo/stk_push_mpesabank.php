// stk_push_mpesabank.php (Full corrected file; with user lookup)
<?php
...
$branch_id = $data['branch_id'] ?? null;  // New: From Flutter
...
// User lookup (corrected)
$userStmt = $conn->prepare("SELECT user_id, branch_id FROM users WHERE phone = ? OR email = ?");
...
if ($user) {
    ...
    $insert = $conn->prepare("
        INSERT INTO users (phone, email, branch_id, credit_score, total_transactions, total_amount)
        VALUES (?, ?, ?, 300, 0, 0)
    ");
    $insert->execute([$phone, $email, $branch_id]);
...
if (isset($resp['ResponseCode']) && $resp['ResponseCode'] == '0') {
    $stmt = $conn->prepare("
        ...
        (user_id, branch_id, email, merchant_request_id, checkout_request_id, amount, recipient_bank_code, recipient_account, recipient_bank_name, phone, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
    ");
    $stmt->execute([$user_id, $branch_id, $email, $resp['MerchantRequestID'], $resp['CheckoutRequestID'], $amount, $bank_code, $account, $bank_name, $phone]);
}
echo $response;
?>
