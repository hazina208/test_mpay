...
if (isset($_POST['register'])) {
    ...
    $entity = trim($_POST['ico'] ?? '');
	$entity_name = trim($_POST['co_name'] ?? '');
    $pos = trim($_POST['position'] ?? '');
    $e_add = trim($_POST['email'] ?? '');
    $pwd = $_POST['pass'] ?? '';
    $role = trim($_POST['role'] ?? '');
    // Basic input validation
    if (empty($fname) || empty($mname) || empty($lname) || empty($telphone) || empty($entity) || empty($entity_name) || 
        empty($pos) || empty($e_add) || empty($pwd) || empty($role)) {
        $_SESSION['status'] = "All fields are required.";
        $_SESSION['status_code'] = "error";
        header("Location: reg_insurance_admin.php");
        exit();
    }
    ...
    try {
        ...
        $stmt->bindParam(5, $entity, PDO::PARAM_STR);
		$stmt->bindParam(6, $entity_name, PDO::PARAM_STR);
    ...
    <div class="black-fill"><br /><br />
        <div class="d-flex justify-content-center align-items-center flex-column">
            <form class="login" method="POST">
                ...
                <div class="mb-3">
                    <label hidden class="form-label">Insurance</label>
                    <input type="hidden" class="form-control" value="Insurance Company" name="ico" required>
                </div>
				<div class="mb-3">
            		...
            		$result = $conn->query("SELECT company FROM insurance_companies");
            		?>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" placeholder="Insurance Admin" readonly>
                    <input type="hidden" class="form-control" name="role" value="7">
                </div>
